...
class MLrawDB(PythonOperator):
    ...
    def _run(self):
        ...
        try:
            ...
            logging.info(f"🏆 Meilleur modèle : {best['model_name']} score {best['score']}")

            # Retourne un XCom (retour natif de PythonOperator)
            return {"model_name": best["model_name"], "score": best["score"]}

        except Exception as e:
            raise AirflowFailException(f"Erreur MLTaskDB {self.task_id}: {e}")