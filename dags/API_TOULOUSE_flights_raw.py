...
from app.datasets import raw_flights_toulouse_done, raw_scheduled_flights_toulouse_done
...
# Définition du DAG
DAG_ID = "API_TOULOUSE_flights_raw"
LIBELLE = "Import des datas vol pour les étapes Raw et Staging via API"
DESCRIPTION = "Import des données de vol en partance de TOULOUSE depuis une API externe (Air France) vers la base de données flight_dw."
...
with DAG(
    ...
    schedule="20 5,7,9,11,13,15,17,19 * * *",
    tags=["API", "FLIGHTS", "IMPORT", "RAW", "TOULOUSE",],
    catchup=False,
    max_active_runs=1,
    dagrun_timeout=timedelta(minutes=15),
    description=DESCRIPTION,
    doc_md="""
            Import des données de vol du jour (scheduled) et de la veille (raw) à l'origine de TOULOUSE depuis une API externe vers la base de données flight_dw.
        """
) as dag:
    ...
    with TaskGroup('loading') as loading:
        ...
        task_load_raw_to_db = Parquet_to_snapshot2(
            ...
            outlets=[raw_flights_toulouse_done],
        ...
        task_load_scheduled_to_db = Parquet_to_snapshot2(
            ...
            database_conn_id="flight_dw_postgres",
            outlets=[raw_scheduled_flights_toulouse_done],
