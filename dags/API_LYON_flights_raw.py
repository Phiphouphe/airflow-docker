...
from app.datasets import raw_flights_lyon_done, raw_scheduled_flights_lyon_done
...
# Définition du DAG
DAG_ID = "API_LYON_flights_raw"
LIBELLE = "Import des datas vol pour les étapes Raw et Staging via API"
DESCRIPTION = "Import des données de vol en partance de LYON depuis une API externe (Air France) vers la base de données flight_dw."
...
with DAG(
    ...
    schedule="5 5,7,9,11,13,15,17,19 * * *",
    tags=["API", "FLIGHTS", "IMPORT", "RAW", "LYON",],
    catchup=False,
    max_active_runs=1,
    dagrun_timeout=timedelta(minutes=15),
    description=DESCRIPTION,
    doc_md="""
            Import des données de vol du jour (scheduled) et de la veille (raw) à l'origine de LYON depuis une API externe vers la base de données flight_dw.
        """
) as dag:
    ...
    with TaskGroup('loading') as loading:
        ...
        task_load_scheduled_to_db = Parquet_to_snapshot2(
            ...
            database_conn_id="flight_dw_postgres",
            outlets=[raw_scheduled_flights_lyon_done],
